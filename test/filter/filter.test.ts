...
describe('pathobject', async () => {
  ...
  it('filter object - subresource filtering - in list', async () => {
    const query = `
      {
        test: resources_v1(filter: {name: "resource H"}) {
          name
          reference_list(filter: {name: {in: ["resource A", "resource B"]}}) {
            name
          }
        }
      }
    ...
    resp.body.data.test.length.should.equal(1);
    resp.body.data.test[0].name.should.equal('resource H');
    resp.body.data.test[0].reference_list.length.should.equal(2);
    resp.body.data.test[0].reference_list[0].name.should.equal('resource A');
    resp.body.data.test[0].reference_list[1].name.should.equal('resource B');
  });
});
